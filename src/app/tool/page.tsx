  ...
  return (
    <div className="min-h-screen bg-brand-charcoal flex flex-col">
      ...
      <header className="bg-brand-dark border-b border-white/10 px-6 py-3.5 flex items-center gap-4 sticky top-0 z-20">
        <Link
          href="/dashboard"
          className="flex items-center gap-1.5 text-sm text-gray-400 hover:text-brand-off-white transition-colors font-medium"
        >
          <ChevronLeft size={16} />
          Dashboard
        </Link>
        ...
        <button
          ...
          className="ml-4 flex items-center gap-1.5 text-xs text-gray-400 hover:text-brand-off-white border border-white/20 hover:border-white/40 px-3 py-1.5 rounded-full transition-colors font-medium"
        >
          <Info size={13} />
          Impact Guide
        </button>
        ...
        <div className="ml-auto flex items-center gap-3">
          <div className="flex items-center gap-5 text-sm">
            <span className="text-gray-400 font-medium">
              <strong className="text-brand-off-white font-black">{totalOffers}</strong> offers mapped
            </span>
            {totalGaps > 0 && (
          ...
          <button className="flex items-center gap-1.5 text-sm text-gray-300 hover:text-brand-off-white border border-white/20 hover:border-white/40 px-3 py-2 rounded-full transition-colors font-medium">
            <Download size={14} />
            Export
          </button>
          <button className="flex items-center gap-1.5 text-sm font-black uppercase tracking-wider bg-brand-gold hover:bg-brand-gold-light text-brand-charcoal px-5 py-2 rounded-full transition-all">
            <Save size={14} />
            Save
          </button>
        </div>
      ...
      {/* ─── Impact Guide Panel ─── */}
      {showInfo && (
        <div className="bg-brand-dark border-b border-white/10 px-6 py-5">
              ...
              {[
                ...
                return (
                  <div key={item.label} className="flex gap-3">
                    ...
                    <div>
                      <div className="text-brand-off-white text-xs font-black mb-1">{item.label}</div>
      ...
      {/* ─── Page title bar ─── */}
      <div className="bg-brand-dark border-b border-white/10 px-6 py-4">
        <h1 className="font-display text-lg font-black text-brand-off-white">Map Your Offer Stack</h1>
                ...
                <div className="flex-1 space-y-3">
                  {tier.offers.map((offer) => (
                    <div
                      ...
                      className={`bg-brand-dark rounded-xl border p-4 cursor-pointer transition-all hover:shadow-md ${
                    ...
                    >
                      <div className="flex items-start justify-between mb-2.5">
                        <h4 className="text-sm font-black text-brand-off-white leading-snug flex-1 pr-2">
                          {offer.name}
                        </h4>
                        <button
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="inline-flex items-center gap-1 text-xs font-black text-green-400 bg-green-900/30 px-2 py-0.5 rounded-full border border-green-700/30">
                          <DollarSign size={10} />
                          {offer.price}
                        </span>
                        <span className="text-xs text-gray-400 bg-white/5 px-2 py-0.5 rounded-full border border-white/10 font-medium">
                          {offer.format}
                        </span>
                      </div>
                      {offer.description && (
                  ...
                  {tier.offers.length === 0 && (
                    <div className="border-2 border-dashed border-brand-gold/40 bg-brand-gold/5 rounded-xl p-5 text-center">
                      ...
                      <div className="text-xs font-black text-brand-off-white mb-0.5">Gap Identified</div>
                  ...
                  <button
                    onClick={() => openAddModal(tier.id)}
                    className="w-full flex items-center justify-center gap-2 py-2.5 text-sm text-gray-500 hover:text-brand-off-white hover:bg-brand-dark border border-transparent hover:border-white/10 rounded-xl transition-all font-medium"
                  >
                    <Plus size={14} />
                    Add offer
                  </button>
      ...
      {/* ─── Offer detail panel ─── */}
      {selectedOffer && (
        <aside className="fixed right-0 top-0 h-full w-80 bg-brand-dark border-l border-white/10 shadow-2xl z-30 flex flex-col">
          <div className="flex items-center justify-between px-6 py-5 border-b border-white/10">
            <h3 className="font-display font-black text-brand-off-white">Offer Details</h3>
            <button
              onClick={() => setSelectedOffer(null)}
              className="p-1 text-gray-400 hover:text-brand-off-white rounded-lg transition-colors"
            >
              <X size={18} />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
            <div>
              <label className="text-xs font-black text-gray-500 uppercase tracking-wider">Offer Name</label>
              <div className="mt-1 text-sm font-black text-brand-off-white">{selectedOffer.offer.name}</div>
            </div>
            <div>
              <label className="text-xs font-black text-gray-500 uppercase tracking-wider">Price Point</label>
              <div className="mt-1 text-2xl font-black text-green-400">{selectedOffer.offer.price}</div>
            </div>
            <div>
              <label className="text-xs font-black text-gray-500 uppercase tracking-wider">Delivery Format</label>
              <div className="mt-1 text-sm text-gray-300 font-medium">{selectedOffer.offer.format}</div>
            </div>
            {selectedOffer.offer.description && (
              <div>
                ...
                <div className="mt-1 text-sm text-gray-400 leading-relaxed font-medium">{selectedOffer.offer.description}</div>
              </div>
            )}

            {/* Money model impact */}
            <div className="bg-brand-charcoal rounded-xl p-4">
          ...
          <div className="px-6 py-5 border-t border-white/10 space-y-2">
            <button className="w-full flex items-center justify-center gap-2 text-sm font-bold text-brand-off-white border border-white/10 hover:border-white/20 hover:bg-white/5 px-4 py-2.5 rounded-xl transition-colors">
              <Edit3 size={14} />
              Edit Offer
            </button>
            <button
              onClick={() => removeOffer(selectedOffer.tierId, selectedOffer.offer.id)}
              className="w-full flex items-center justify-center gap-2 text-sm font-bold text-red-400 hover:bg-red-900/20 px-4 py-2.5 rounded-xl transition-colors"
            >
              <Trash2 size={14} />
              Remove Offer
            </button>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-brand-dark rounded-2xl shadow-2xl w-full max-w-md border border-white/10">
            <div className="flex items-center justify-between px-6 py-5 border-b border-white/10">
              <div>
                <h3 className="font-display font-black text-brand-off-white text-lg">Add New Offer</h3>
                {activeTierId && (
                  <p className="text-sm text-gray-500 mt-0.5 font-medium">
            ...
            <div className="px-6 py-5 space-y-4">
              <div>
                <label className="block text-sm font-black text-brand-off-white mb-1.5">
                ...
                <input
                  ...
                  className="w-full bg-brand-charcoal border border-white/10 rounded-xl px-4 py-2.5 text-sm text-brand-off-white placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-brand-gold/40 focus:border-brand-gold transition-colors font-medium"
                  autoFocus
                />
              </div>

              <div>
                <label className="block text-sm font-black text-brand-off-white mb-1.5">Price Point</label>
                <input
                  type="text"
                  value={form.price}
              ...
              <div>
                <label className="block text-sm font-black text-brand-off-white mb-1.5">Delivery Format</label>
                <select
                  value={form.format}
                  onChange={(e) => setForm((f) => ({ ...f, format: e.target.value }))}
                  className="w-full bg-brand-charcoal border border-white/10 rounded-xl px-4 py-2.5 text-sm text-brand-off-white focus:outline-none focus:ring-2 focus:ring-brand-gold/40 focus:border-brand-gold transition-colors font-medium"
                >
                  {FORMATS.map((f) => (
              ...
              <div>
                <label className="block text-sm font-black text-brand-off-white mb-1.5">Description</label>
                <textarea
                  value={form.description}
                  onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))}
                  placeholder="What does this offer include? Who is it for?"
                  rows={3}
                  className="w-full bg-brand-charcoal border border-white/10 rounded-xl px-4 py-2.5 text-sm text-brand-off-white placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-brand-gold/40 focus:border-brand-gold transition-colors resize-none font-medium"
                />
              </div>
            </div>
            ...
            <div className="flex gap-3 px-6 pb-6">
              <button
                onClick={() => setModalOpen(false)}
                className="flex-1 border border-white/10 text-brand-off-white font-bold py-3 rounded-full text-sm hover:bg-white/5 transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={addOffer}
                disabled={!form.name}
                className="flex-1 bg-brand-gold hover:bg-brand-gold-light disabled:bg-gray-700 disabled:text-gray-500 text-brand-charcoal font-black uppercase tracking-wider py-3 rounded-full text-sm transition-all"
              >
                Add Offer
              </button>
            </div>
